...
public class LoadCSVHandler implements Route {
   ...
   *
   * @param data GlobalData of the parsed CSV file
   */
  ...
  /**
   * This method handles the loadcsv query from the web URL
   *
  ...
  public Object handle(Request request, Response response) {
    ...
    try {
      ...
      responseMap.put("status", "error_bad_request");
      responseMap.put(
          "message",
          "Missing parameter(s). Make sure to type in the full file path and the header using Yes or No.");
      return adapter.toJson(responseMap);
    }

    // if the header input is invalid
    if (!header.equals("Yes") && !header.equals("No")) {
      this.responseMap.put("status", "error_bad_request");
    ...
    try {
      ...
      if (filepath.contains(
          "/Users/emilyhong/Desktop/cs0320/server-mlau17-elhong/src/main/java/edu/brown/cs/student/main/data")) {
        this.filepath = filepath;
        FileReader reader = new FileReader(filepath);
        this.parser = new CSVParser<>(new StringCreator(), this.header, reader);
        // set the global variable to the new data
        List<List<String>> newData = this.parser.sortData();
        this.data.setCsvData(newData);
        ...
        this.responseMap.put(
            "message", "No filepath found. Please enter the full or correct file path.");
      }
   ...
   *
   * @return CSVParser used to load the file
   *  */
